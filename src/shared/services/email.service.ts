...
const transporter = nodemailer.createTransport({
  ...
  auth: {
    ...
    // .replace(/\s/g, "") elimina los espacios que Google a veces pone en la contraseña
...
export const emailService = {
  enviarEmailRecuperacion: async ({ to, resetLink, nombreUsuario }: EnviarEmailResetProps) => {
    const from = process.env.EMAIL_FROM || `"Tu Marca" <${process.env.SMTP_USER}>`;
    ...
    try {
      await transporter.sendMail({
        from,
        to,
        subject: "Recuperación de contraseña - Tu Marca",
        html: `
          <div style="font-family: 'Helvetica Neue', Arial, sans-serif; max-width: 500px; margin: 0 auto; background-color: #FAFAF7; padding: 20px; border-radius: 12px;">
            <div style="background-color: #1C1C1C; padding: 20px; text-align: center; border-radius: 12px 12px 0 0;">
              <h1 style="margin: 0; color: #C9A96E; font-size: 24px; font-weight: 500;">Tu Marca</h1>
            </div>
